...
<div class="container mt-4">
    ...
    <form action="{{ route('admin.userprofprivileges.update', [
        'profile'   => $userprofprivilege->UserProfileID,
        'privilege' => $userprofprivilege->UserPrivilegesID
    ]) }}" method="POST">
        @csrf
        @method('PUT')
            ...
            <select class="form-select @error('UserProfileID') is-invalid @enderror" id="UserProfileID" name="UserProfileID">
                ...
                @foreach($userProfiles as $profile)
                    <option value="{{ $profile->UserProfileID }}"
                        {{ $userprofprivilege->UserProfileID == $profile->UserProfileID ? 'selected' : '' }}>
            ...
            <select class="form-select @error('UserPrivilegesID') is-invalid @enderror" id="UserPrivilegesID" name="UserPrivilegesID">
                ...
                @foreach($privileges as $priv)
                    <option value="{{ $priv->LookupID }}"
                        {{ $userprofprivilege->UserPrivilegesID == $priv->LookupID ? 'selected' : '' }}>
                        {{ $priv->LookupValue }}
                    </option>
                @endforeach
            </select>
